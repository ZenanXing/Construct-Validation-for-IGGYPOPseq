...
## Index info.
df_index <- df_input %>% 
  dplyr::select(SampleID, FwBarcode, FwPrimer, RvBarcode, RvPrimer) %>% 
  arrange(SampleID)
colnames(df_index) <- c("SampleID", "Fwindex", "FwPrimer", "Rvindex", "RvPrimer")
write.table(df_index, paste0(output_dir, "/Analysis_Results/InputFiles/IndexCombination.txt"), sep = "\t", row.names = FALSE, quote = FALSE)

## Reference info.
df_ref <- df_input %>% 
  dplyr::select(SampleID, ReferenceName) %>% 
  mutate(ReferenceFile = paste0(ReferenceName, ".fasta")) %>% 
...
df_ref <- df_input %>% dplyr::select(ReferenceName, ReferenceSequence) %>% distinct()
...
for (i in 1:nrow(df_ref)) {
  ...
  names(dna_sequences) <- df_ref$ReferenceName[i]
  # Writing the DNAStringSet to a FASTA file
  writeXStringSet(dna_sequences, filepath = paste0(output_dir, "/Analysis_Results/InputFiles/references/", df_ref$ReferenceName[i], ".fasta"))
}
...
if (isTRUE(aa_validation)) {
  ...
  df_orf <- df_input %>% 
    ...
    dplyr::select(ReferenceName, Search_length) %>% 
    distinct()
...
df_config <- df_input %>% 
  ...
  dplyr::select(ArrayTaskID, SampleID, ReferenceName)
...
df_set <- df_input %>% 
  ...
  dplyr::select(Set, ReferenceName) %>% 
  distinct()
