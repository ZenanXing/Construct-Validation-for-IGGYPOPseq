...
p <- ggplot(df_plot_f, aes(x = n_frags, y = n_genes, fill = Catgry, group = Catgry)) + 
  ...
  scale_fill_manual(values = c("complete" = "#A6CEE3", "total" = "#1F78B4"), breaks = c("complete", "total")) +
  theme_bw() +
...
### Correct sequence vs Number of fragments
```{r echo = FALSE, results = 'asis'}
...
p <- ggplot(df_plot_f, aes(x = n_frags, y = n_genes, fill = Catgry, group = Catgry)) + 
  ...
  ylab("Number of Genes") +
  ylim(0, max(df_plot_f$n_genes) + 5) +
  scale_fill_manual(values = c("correct" = "#B2DF8A", "total" = "#33A02C"), breaks = c("correct", "total")) +
  theme_bw() +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank()) + 
  theme(axis.text= element_text(size = 16),
...
p <- ggplot(df_plot, aes(x = n_frags, y = n, fill = Assembly_Status, group = Assembly_Status)) + 
  ...
  ylab("Clones (n)") +
  #ylim(0, max(df_plot_f$n_genes) + 5) +
  scale_fill_manual(values = c("error_free" = "#B2DF8A", "snp_indel" = "#A6CEE3", "unassembled" = "#cdcdcd")) +
  theme_bw() +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank()) + 
  theme(axis.text= element_text(size = 16),
...
df_plot_2 <- df_exp %>% 
  dplyr::select(primer_index, SampleID, Assembly_Status) %>%
  mutate(Assembly_Status_f = ifelse(Assembly_Status=="unassembled", NA, "Assembled")) %>% 
  drop_na() %>% 
...
p <- ggplot(df_plot, aes(x = n_clones, y = n_constructs, fill = Assembly_Status_f)) + 
  geom_bar(stat = "identity") +
  #geom_text(aes(y = n_genes, label = lab, hjust = -0.1), position = position_dodge(width = 0.9), vjust = 0.5, size = 4) +
  facet_wrap(.~Assembly_Status_f, ncol = 1) +
  ylab("Constructs (n)") +
  xlab("Clones/Construct (n)") +
  #ylim(0, max(df_plot_f$n_genes) + 5) +
  scale_fill_manual(values = c("Error Free" = "#B2DF8A", "Assembled" = "#A6CEE3")) +
  theme_bw() +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank()) + 
  theme(axis.text= element_text(size = 16),
